...
public class AnimalGenerator : MonoBehaviour
{
    ...
    public float cameraThreshold = 5f; // カメラを上げるしきい値
    public float cameraMoveAmount = 1f; // 上げる量
    ...
    {
    mainCamera.transform.Translate(0, amount, 0);
    pivotHeight += amount;
    }
    ...
    {   
        ...
        if (geneAnimal != null)
            {
                float animalY = geneAnimal.transform.position.y;
                if (animalY > cameraThreshold)
                {
                    //MoveCameraUp(cameraMoveAmount);
                    //cameraThreshold += cameraMoveAmount; // 次のしきい値を更新
                    //Debug.Log("カメラを上げました。新しいしきい値: " + cameraThreshold);
                }
            }
    ...
    IEnumerator GenerateAnimal()
    {
        ...
        isGene = true; // ← ここで設定することで、確実に1回だけ生成される
    }
